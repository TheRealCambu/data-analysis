...
def plot_multiple_ber(
    ...
    label_dict = {
        "ber": {"title": "BER", "ylabel": "BER"},
        "evm": {"title": "EVM", "ylabel": r"EVM$_{\mathrm{m}}$ [%]"},
        "ber_evm": {"title": "BER$_{\mathrm{EVM}}$", "ylabel": r"BER$_{\mathrm{EVM}}$"},
    }
    ...
    for idx, (data_vector, x_idx, x_data, legend_label) in enumerate(zip(
        ...
        if plot_max:
            mask = np.isfinite(data_max) & (data_max <= filter_threshold)
    ...
    plt.title(
        f"{'Maximum filtered' if plot_max else 'Average'} {label_info['title']} vs ROP {extra_title_label}"
    )
    plt.legend(loc="best")
